...
// the receipt. Shared by a membership, a fee and a single entry. `fixedAmount` = the amount cannot be
// changed (a fee is paid in full).

export default function PayForm({ defaultAmountCents, fixedAmount, busy, onPay, onCancel }: {
    defaultAmountCents: number; fixedAmount?: boolean; busy: boolean;
        ...
        <div className="rounded-xl border border-slate-200 p-3 space-y-2">
            ...
            <div className="flex gap-2">
                <button type="button" disabled={busy || cents <= 0} onClick={() => onPay({ amount_cents: cents, method, send_receipt: receipt })}
                    className="inline-flex items-center gap-2 min-h-10 px-4 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-bold disabled:opacity-40">
                    {busy && <Loader2 className="w-4 h-4 animate-spin" aria-hidden />} רישום תשלום
                </button>
                <button type="button" onClick={onCancel} className="min-h-10 px-3 text-sm text-slate-600">ביטול</button>
            </div>
            <p className="text-xs text-slate-500">המערכת רושמת את התשלום ומפיקה קבלה — היא לא גובה כסף.</p>
        </div>
    );
}
