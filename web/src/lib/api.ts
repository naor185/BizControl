...
export async function downloadExpensesPdf(month: number, year: number): Promise<void> {
    const token = typeof window !== "undefined" ? localStorage.getItem("bizcontrol_token") : null;
    const url = `${API_BASE}/api/expenses/export/pdf?month=${month}&year=${year}`;
    const res = await fetch(url, { headers: token ? { Authorization: `Bearer ${token}` } : {} });
    if (!res.ok) {
        const data = await res.json().catch(() => null);
        throw new Error(data?.detail || "שגיאה בהפקת ה-PDF");
    }
    const blob = await res.blob();
    const a = document.createElement("a");
    a.href = URL.createObjectURL(blob);
    a.download = `expenses_${year}_${String(month).padStart(2, "0")}.pdf`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
}
